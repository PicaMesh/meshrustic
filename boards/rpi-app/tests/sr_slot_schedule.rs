...
use mesh_routing::{
    decode_packed_neighbors, write_packed_header, NeighborGraph, PackedNeighbor, EdgeSource,
    DEVICE_ROLE_REPEATER, DEVICE_ROLE_ROUTER, POOR_LINK_ETX_THRESHOLD,
};
...
fn best_candidate_assigned_earlier_slot() {
    ...
    graph.set_device_role(DEVICE_ROLE_ROUTER);
    graph.observe_direct_neighbor(BB, -70, 8, 0, 0);
    graph.observe_direct_neighbor(EE, -72, 7, 0, 0);
    ...
    let plan = graph.plan_broadcast_relay(0x98, BB, BB, 0xFFFF_FFFF, 0, HALF);
    assert!(plan.should_relay);
